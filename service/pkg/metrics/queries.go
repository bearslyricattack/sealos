// Package metrics provides query templates for various database types.
package metrics

// QueryTemplates contains PromQL query templates for different database types and metrics.
// Template variables:
//
//	# - will be replaced with namespace
//	@ - will be replaced with cluster/app name
var QueryTemplates = map[string]map[string]string{
	...
	"apecloud-mysql": {}, // Will be populated from mysql
...
func init() {
	// apecloud-mysql uses the same queries as mysql
	QueryTemplates["apecloud-mysql"] = QueryTemplates["mysql"]
}
